            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Optional(CONF_IMAGE_SERVER_URL, default=""): cv.string,
                    vol.Required(CONF_IMAGE_COUNT, default=5): NumberSelector(
                        NumberSelectorConfig(
                            mode=NumberSelectorMode.BOX, min=1, max=20, step=1
                        )
                    ),
                }
            ),
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Optional(
                        CONF_IMAGE_SERVER_URL,
                        default=self._config_entry.data.get(CONF_IMAGE_SERVER_URL, ""),
                    ): cv.string,
                    vol.Required(
                        CONF_IMAGE_COUNT,
                        default=int(self._config_entry.data.get(CONF_IMAGE_COUNT, 5)),
                    ): NumberSelector(
                        NumberSelectorConfig(
                            mode=NumberSelectorMode.BOX, min=1, max=20, step=1
                        )
