...
_formatName(entity, fallbackKey) {
  ...
  _captureScrollState() {
    ...
    // Bubble Card's mobile popup uses a scrollable ancestor outside this
    // card (and sometimes across a Shadow DOM boundary). Replacing this
    // card's shadowRoot can make WebKit reset that ancestor to scrollTop=0.
    // Keep the position of every actually-scrolled composed ancestor.
    ...
    if (
      ...
      !positions.some(position => position.node === scrollingElement)
    ) {
      positions.push({
  ...
  _restoreScrollState(positions) {
    if (!positions?.length) return;

    const restore = () => {
      positions.forEach(({ node, top, left }) => {
        if (!node) return;
        if (node !== document.scrollingElement && node.isConnected === false) return;

        if (typeof node.scrollTop === "number") node.scrollTop = top;
        if (typeof node.scrollLeft === "number") node.scrollLeft = left;
      });
    };

    // Restore immediately, then after WebKit/Bubble Card has completed its
    // layout passes. The second RAF is important on iOS Home Assistant.
    restore();
    requestAnimationFrame(() => {
      restore();
  ...
  _update() {
    ...
    // so a full render is still required. Preserve the enclosing popup's scroll
    // position because replacing shadowRoot.innerHTML can reset it on mobile.
